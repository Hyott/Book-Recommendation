...
# ### flutter로 부터 책 선택을 했다는 신호를 받고 + user_resonse에서 갱신된 question_number가 있음을 확인하고 그 선택값을 조회함.
# if get_choice_bool(user_id, question_number):
#     book_choice_updated = choice_arrange()
#     book_a, book_b = suggest_books(book_embeddings, cluster_to_books, noise_factor)
#     message_a = get_message_by_id(ids, ids[book_a], book_data)
#     message_b = get_message_by_id(ids, ids[book_b], book_data)

### flutter로부터 question_number == 10 이며 모든 셀렉이 완료되었다고 알게된다면,
